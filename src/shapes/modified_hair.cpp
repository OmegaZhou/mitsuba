 ...
 */
    ...
    class ModifiedHairKDTree : public SAHKDTree3D<ModifiedHairKDTree> {
    friend class GenericKDTree<AABB, SurfaceAreaHeuristic3, ModifiedHairKDTree>;
    friend class SAHKDTree3D<ModifiedHairKDTree>;
    public:
        using SAHKDTree3D<ModifiedHairKDTree>::IndexType;
        using SAHKDTree3D<ModifiedHairKDTree>::SizeType;

        ModifiedHairKDTree(std::vector<Point>& vertices,
...
ModifiedHairShape::ModifiedHairShape(const Properties& props) : Shape(props) {
    ...
    m_interpolation_thredhold = props.getFloat("interpolation_thredhold", 0.1f);
    ...
    m_kdtree = new ModifiedHairKDTree(vertices, vertexStartsFiber, radius);
}

ModifiedHairShape::ModifiedHairShape(Stream* stream, InstanceManager* manager)
    : Shape(stream, manager) {
    Float radius = stream->readFloat();
    m_interpolation_thredhold = stream->readFloat();
...
void ModifiedHairShape::serialize(Stream* stream, InstanceManager* manager) const {
    ...
    stream->writeFloat(m_interpolation_thredhold);
...
bool ModifiedHairShape::rayIntersect(const Ray& ray, Float mint,
    Float maxt, Float& t, void* temp) const {
    return m_kdtree->rayIntersect(ray, mint, maxt, t, temp);
}

bool ModifiedHairShape::rayIntersect(const Ray& ray, Float mint, Float maxt) const {
    return m_kdtree->rayIntersect(ray, mint, maxt);
}

void ModifiedHairShape::fillIntersectionRecord(const Ray& ray,
    ...
    const ModifiedHairKDTree::IntersectionStorage* storage =
        static_cast<const ModifiedHairKDTree::IntersectionStorage*>(temp);
    ModifiedHairKDTree::IndexType iv = storage->iv;
    its.p = storage->p;

    Vector axis = m_kdtree->tangent(iv);
    Vector relHitPoint = its.p - m_kdtree->firstVertex(iv);
    ...
    its.instance = this;
    its.time = ray.time;
    if (m_kdtree->prevSegmentExists(iv)) {
        float segment_len = (m_kdtree->secondVertex(iv) - m_kdtree->firstVertex(iv)).length();
        float p_len = std::abs(dot(its.p - m_kdtree->firstVertex(iv), axis));
        float ratio = p_len / segment_len;
        if (ratio < m_interpolation_thredhold) {
            const Vector axis2 = m_kdtree->tangent(iv - 1);
            float u = (1 - ratio / m_interpolation_thredhold) * 0.5f;
            axis = normalize((1 - u) * axis + u * axis2);
            its.geoFrame.s = axis;
            its.geoFrame.n = Normal(normalize(relHitPoint - dot(axis, relHitPoint) * axis));
            its.geoFrame.t = cross(its.geoFrame.n, its.geoFrame.s);
        }
    }
    if (m_kdtree->nextSegmentExists(iv)) {
        float segment_len = (m_kdtree->secondVertex(iv) - m_kdtree->firstVertex(iv)).length();
        float p_len = std::abs(dot(its.p - m_kdtree->secondVertex(iv), axis));
        float ratio = p_len / segment_len;
        if (ratio < m_interpolation_thredhold) {
            const Vector axis2 = m_kdtree->tangent(iv + 1);
            float u = (1 - ratio / m_interpolation_thredhold) * 0.5f;
        ...
        //relHitPoint = its.p - m_kdtree->secondVertex(iv);
    }
}

ref<TriMesh> ModifiedHairShape::createTriMesh() {
    ...
    uint32_t hairIdx = 0;
    for (ModifiedHairKDTree::IndexType iv = 0; iv < (ModifiedHairKDTree::IndexType)hairVertices.size() - 1; iv++) {
...
const KDTreeBase<AABB>* ModifiedHairShape::getKDTree() const {
    return m_kdtree.get();
}

const std::vector<Point>& ModifiedHairShape::getVertices() const {
    return m_kdtree->getVertices();
}

const std::vector<bool>& ModifiedHairShape::getStartFiber() const {
    return m_kdtree->getStartFiber();
}

AABB ModifiedHairShape::getAABB() const {
    return m_kdtree->getAABB();
}

size_t ModifiedHairShape::getPrimitiveCount() const {
    return m_kdtree->getHairCount();
}

size_t ModifiedHairShape::getEffectivePrimitiveCount() const {
    return m_kdtree->getHairCount();
}

Float ModifiedHairShape::getSurfaceArea() const {
    Log(EError, "ModifiedHairShape::getSurfaceArea(): Not implemented.");
    return -1;
}

std::string ModifiedHairShape::toString() const {
...
MTS_IMPLEMENT_CLASS(ModifiedHairKDTree, false, KDTreeBase)
MTS_IMPLEMENT_CLASS_S(ModifiedHairShape, false, Shape)
MTS_EXPORT_PLUGIN(ModifiedHairShape, "Hair intersection shape");
MTS_NAMESPACE_END
